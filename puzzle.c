...
void search(char **grid,int nRow,int nCol,char *word){
  ...
  for(y=0;y<nRow&&!found;y++){
    for(x=0;x<nCol&&!found;x++){
      if(grid[y][x]==word[0]){
        if(nCol-x-1>=i&&grid[y][x+1]==word[1]){ //check right
	  for(l=2;l<=i;l++){
	...
	}else if(nCol-x-1>=i&&nRow-y-1>=i&&grid[y+1][x-1]==word[1]){//check downleft
	  for(l=2;l<=i;l++){
	      if(y+l<nRow&&x-l>=0){
	...
	}else if(nRow-y-1>=i&&grid[y+1][x]==word[1]){//check down
	  for(l=2;l<=i;l++){
	    if(y+l<nRow){
	...
	}else if(nCol-x-1>=i&&nRow-y-1>=i&&grid[y+1][x+1]==word[1]){//check downright
	  for(l=2;l<=i;l++){
	...
	if(nCol-x-1>=i&&grid[y][x+1]==word[i-1]){ //check right
	  for(l=2;l<=i;l++){
	...
	}else if(x>=i&&nRow-y-1>=1&&grid[y+1][x-1]==word[i-1]){//check downleft
	  for(l=2;l<=i;l++){
	      if(y+l<nRow&&x-l>=0){
	...
	}else if(nCol-x-1>=i&&nRow-y-1>=i&&grid[y+1][x+1]==word[i-1]){//check downright
	  for(l=2;l<=i;l++){
	      if(y+l<nRow&&x+l<nCol){
		if(grid[y+l][x+l]!=word[i-l]){
		  break;
